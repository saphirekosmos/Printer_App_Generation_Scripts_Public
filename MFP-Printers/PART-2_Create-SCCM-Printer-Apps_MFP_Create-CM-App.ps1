...
# WARNING: You will need to update these for your own setup:
    ...
    #Around Line 115: $DetectScript = "foreach ($Key in (Get-ChildItem `"HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Print\Connections\`")) { if ((Get-ItemProperty -Path $key.PSPath -Name `"Printer`").`"Printer`" -eq `"\\MFPPRINTSERVER\$PrinterName`") {Write-Output `"True`"}}"
...
foreach ($Printer in $Printers) {
    ...
    $IconPath = "$FileSource\Source Files\MFP-Printer.png"
    ...
    $RuntimeEstimated = 1
    #Specifies any dependencies for the app. This is the app that installs the drivers.
    $AppDependency = "Print Drivers: MFP"
    ...
    $DetectScript = "foreach ($Key in (Get-ChildItem `"HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Print\Connections\`")) { if ((Get-ItemProperty -Path $key.PSPath -Name `"Printer`").`"Printer`" -eq `"\\MFPPRINTSERVER\$PrinterName`") {Write-Output `"True`"}}"
